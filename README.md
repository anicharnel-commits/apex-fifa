...
### Gemini — gestion des indisponibilités
Le bot utilise Gemini avec un modèle principal et des modèles de secours. Si Gemini renvoie une erreur transitoire (notamment 503 UNAVAILABLE ou 429), le bot essaie automatiquement le modèle suivant au lieu d'abandonner immédiatement l'analyse.

Modèle principal par défaut : `gemini-3.6-flash`
Modèles de secours par défaut : `gemini-3.6-flash-lite`, puis `gemini-3.6-flash`.


## Gemini
The default model is `gemini-3.6-flash`, with `gemini-3.5-flash-lite` and `gemini-3.7-flash` as fallbacks. The bot treats 404 model-unavailable errors like transient model failures and continues with the next configured model.
