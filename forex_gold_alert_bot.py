"""
Forex / Gold 24h Alert Bot — MANUAL CONTROL ONLY
...
def monitor_loop() -> None:
    """Continuous monitoring loop, running in a background thread on Render."""
    ...
    while True:
        ...
        try:
            ...
            logger.exception("Erreur inattendue du calendrier économique.")

        try:
            process_news()
        except Exception:
            logger.exception("Erreur inattendue du module news/Gemini.")
...
def main() -> None:
    ...
    logger.info("==============================================")
    logger.info("Forex / Gold Alert Bot — RENDER FREE READY")
    logger.info("CONTRÔLE MT5 100%% MANUEL")
    logger.info("Port HTTP: %d", WEB_PORT)
    ...
    # Monitoring runs in a background thread while Flask keeps the Render Web
    # Service alive and exposes /health. Render Free may still spin down the
    # service when it receives no inbound traffic; this does not guarantee 24/7.
    worker = threading.Thread(target=monitor_loop, name="monitor-loop", daemon=True)
    worker.start()
